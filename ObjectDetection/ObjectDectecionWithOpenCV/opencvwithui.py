...
from PyQt5 import uic  # Import uic to load .ui file
...
class TehseenCode(QDialog):
    def __init__(self):
        ...
        self.text_browser.setText('Press "Camera" to connect with camera')
        self.processed_img_label.setScaledContents(True)
        
        # Connect UI buttons to their respective functions
        self.select_excel_btn.clicked.connect(self.upload_excel_file)
        self.upload_img_btn.clicked.connect(self.upload_image)
        self.send_btn.clicked.connect(self.send_data_to_excel)
        self.undo_btn.clicked.connect(self.undo_reset_counter)
        self.reload_btn.clicked.connect(self.reload_app)
        self.quit_btn.clicked.connect(self.quit_app)
        
        self.camera_btn.clicked.connect(self.start_video)
        self.capture_btn.clicked.connect(self.capture_new_image)
        
        # Setup the layout for displaying images
        layout = QHBoxLayout()
        self.frame_right.setLayout(layout)
        layout.addWidget(self.img_label)
        layout.addWidget(self.processed_img_label)
    ...
    def frameCaptured(self):
        self.text_browser.setText('Image captured')
    ...
    def detect_cameras(self):
        ...
        if not camera_indices:
            self.camera_select_combo.addItem("No Cameras Detected")
        else:
            self.camera_select_combo.clear()
            for index in camera_indices:
                self.camera_select_combo.addItem(f"Camera {index}")
    ...
    def get_selected_camera_index(self):
        ...
        if self.thread is not None:
            self.thread.stop()
        return self.camera_select_combo.currentIndex()
    ...
    def camera_failed(self):
        ...
        self.text_browser.setText("Failed to start the camera. Please try selecting a different camera.")
    ...
    def detect_image(self, image_path):
        ...
        self.text_browser.setText(f'Total: {elements} = {self.total_detected_objects}')
        self.total_objects_label.setText(f'Total Objects: {self.total_detected_objects}')
    ...
    def upload_image(self):
        ...
        if file_name:
            ...
            try:
                ...
                QTimer.singleShot(100, lambda: self.display_image(file_name, self.img_label))  
            except Exception as e:
    ...
    def update_processed_image(self, img_path):
        self.display_image(img_path, self.processed_img_label)
    ...
    def write_data_to_excel(self):
        ...
        if selected_cell:
            try:
                ...
                print(f"Error writing data to Excel: {e}")
                self.text_browser.setText('Failed to write data to Excel')
    ...
    def undo_reset_counter(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                self.text_browser.setText(f'Total Objects: {elements} = {self.total_detected_objects}')
                self.total_objects_label.setText(f'Total Objects: {self.total_detected_objects}')
                self.send_data_to_excel()
            except:
    ...
    def reload_app(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            self.processed_img_label.clear()
